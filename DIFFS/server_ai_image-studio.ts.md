# Diff: server/ai/image-studio.ts

```diff
--- original_reference/server/ai/image-studio.ts	2026-07-06 18:23:36.000000000 +0000
+++ audit/server/ai/image-studio.ts	2026-07-06 21:12:57.406622039 +0000
@@ -1,7 +1,6 @@
...
@@ -47,94 +46,33 @@
     ...
     if (provider === "flux") {
-      // --- RUNTIME DATABASE AUDIT LOG ---
-      const sqliteDb = (db as any).getDatabase();
-      const sqliteRows: any[] = [];
-      if (sqliteDb) {
-        try {
-          const stmt = sqliteDb.prepare("SELECT workspace_id, provider, is_enabled FROM workspace_ai_providers");
-          while (stmt.step()) {
-            sqliteRows.push(stmt.getAsObject());
-          }
-          stmt.free();
-        } catch (e: any) {
-          console.error("[AUDIT ERROR] Failed to query sqlite directly:", e.message);
-        }
-      }
-
-      const dbFilePath = process.env.NODE_ENV === "production" ? "/tmp/aurapost.db" : "./storage/aurapost.db";
-      const keyReturned = await db.getAIProviderApiKey(workspaceId, "flux");
-      const nodeEnvValue = process.env.NODE_ENV;
-      const containsFluxRow = sqliteRows.some((r: any) => r.provider === "flux");
-
-      const logLines: string[] = [];
-      logLines.push("=== PROCESS-INTERNAL GENERATION AUDIT ===");
-      logLines.push(`1. workspaceId: ${workspaceId}`);
-      logLines.push(`2. Rows returned by SQLite (workspace_ai_providers): ${JSON.stringify(sqliteRows, null, 2)}`);
-      logLines.push(`3. Database file path in use: ${dbFilePath}`);
-      logLines.push(`4. Value returned by getAIProviderApiKey(): ${keyReturned ? `[EXISTS, len=${keyReturned.length}]` : "null"}`);
-      logLines.push(`5. Is running in production or development: ${nodeEnvValue === "production" ? "production" : "development"}`);
-      logLines.push(`6. Whether NODE_ENV is set: ${nodeEnvValue !== undefined ? `yes (${nodeEnvValue})` : "no"}`);
-      logLines.push(`7. Whether the database contains a flux row at that exact moment: ${containsFluxRow}`);
-      logLines.push("=========================================\n");
-
-      const logText = logLines.join("\n");
-      console.log(logText);
-      try {
-        fs.appendFileSync("./storage/audit.log", logText);
-      } catch (err) {
-        // ignore
-      }
-
...
-      
-      console.log("=== RUNTIME AUDIT LOG ===");
-      console.log("workspaceId:", workspaceId);
-      console.log("provider:", provider);
-      console.log("apiKeyExists:", !isSimulated);
-      console.log("isSimulated:", isSimulated);
-      console.log("BFL endpoint used:", endpoint);
-      console.log("=========================");
-
-      let response: Response;
-      if (isSimulated) {
-        console.log("BFL_REQUEST_POST:", JSON.stringify({
+
...
+        body: JSON.stringify({
           ...
           height: 1024
-        }));
-        const mockBflPostRes = { id: "bfl-task-884c9ad3-bf2d-4567-9382-fa4c49d2abcd" };
-        console.log("BFL_RESPONSE_POST: HTTP 200 OK", JSON.stringify(mockBflPostRes));
-        response = {
-          ok: true,
-          status: 200,
-          statusText: "OK",
-          json: async () => mockBflPostRes,
-          text: async () => JSON.stringify(mockBflPostRes)
-        } as Response;
-      } else {
...
-      console.log("HTTP status from BFL:", response.status);
+        })
+      });
...
-          let mockPollRes;
-          if (pollCount === 1) {
-            mockPollRes = { status: "Processing" };
-          } else {
-            mockPollRes = {
-              status: "Ready",
-              result: {
-                sample: "https://images.unsplash.com/photo-1542291026-7eec264c27ff?auto=format&fit=crop&w=1024&q=80"
-              }
-            };
-          }
-          console.log(`BFL_RESPONSE_GET (Call ${pollCount}): HTTP 200 OK`, JSON.stringify(mockPollRes));
-          response = {
-            ok: true,
-            status: 200,
-            statusText: "OK",
-            json: async () => mockPollRes,
-            text: async () => JSON.stringify(mockPollRes)
-          } as Response;
-        } else {
-          response = await fetch(endpoint, {
