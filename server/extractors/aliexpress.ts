...
async function fetchWithRetry(
  ...
  try {
    ...
    if (
      ...
      logger.warn(
        "[AliExpress] HTTP %d (attempt %d/%d). Retrying in %dms...",
        response.status, attempt, MAX_RETRIES + 1, delay
      );
      await sleep(delay);
    ...
    if ((isTimeout || isNetwork) && attempt <= MAX_RETRIES) {
      ...
      logger.warn(
        "[AliExpress] Network error (attempt %d/%d): %s. Retrying in %dms...",
        attempt, MAX_RETRIES + 1, error.message, delay
      );
      await sleep(delay);
      return fetchWithRetry(url, buildRandomHeaders(), cookies, attempt + 1);
    }
...
function collectAllImages(html: string): string[] {
  ...
  if (nextDataMatch?.[1]) {
    try {
      ...
      if (data) {
        ...
        logger.debug("[AliExpress] Collected %d images from __NEXT_DATA__", images.length);
      }
    } catch (e) {
      logger.debug({ err: e }, "[AliExpress] __NEXT_DATA__ parse failed");
    }
  }
  ...
  if (initStateMatch?.[1]) {
    try {
      ...
      if (data) {
        ...
        logger.debug("[AliExpress] Collected %d images from __INITIAL_STATE__", images.length);
      }
    } catch (e) {
      logger.debug({ err: e }, "[AliExpress] __INITIAL_STATE__ parse failed");
    }
  }
  ...
  if (runParamsMatch?.[1]) {
    try {
      ...
      if (data) {
        ...
        logger.debug("[AliExpress] Collected %d images from runParams", images.length);
      }
    } catch (e) {
      logger.debug({ err: e }, "[AliExpress] runParams parse failed");
    }
  }
  ...
  if (appDataMatch?.[1]) {
    try {
      ...
      if (data) {
        ...
        logger.debug("[AliExpress] Collected %d images from __APP_DATA__", images.length);
      }
    } catch (e) {
      logger.debug({ err: e }, "[AliExpress] __APP_DATA__ parse failed");
    }
  }
  ...
  logger.debug("[AliExpress] Collected %d images from <img> tags", imgCount);

  // 6. OpenGraph and Twitter
  const ogImage = extractMetaContent(html, "og:image");
  if (ogImage && ogImage.startsWith("http")) {
    allUrls.add(ogImage);
    logger.debug("[AliExpress] Collected og:image");
  }
  const twImage = extractMetaContent(html, "twitter:image");
  if (twImage && twImage.startsWith("http") && !allUrls.has(twImage)) {
    allUrls.add(twImage);
    logger.debug("[AliExpress] Collected twitter:image");
  }
  ...
  logger.debug("[AliExpress] Collected %d images from broad alicdn scan", broadCount);
...
export class AliExpressExtractor extends BaseExtractor {
  ...
  private async fetchAliExpressPageEnhanced(url: string): Promise<string> {
    ...
    if (!this.homepageBootstrapped) {
      try {
        logger.info("[AliExpress] Bootstrapping homepage cookies...");
        ...
        if (homeRes.ok) {
          logger.info("[AliExpress] Homepage bootstrap successful, cookies: %d", this.cookieJar.getJar().size);
          this.homepageBootstrapped = true;
          await sleep(randomInt(1_000, 3_000));
        }
      } catch (e) {
        logger.warn({ err: e }, "[AliExpress] Homepage bootstrap failed (non-fatal)");
      }
    }
    ...
    for (let i = 0; i < MAX_RETRIES; i++) {
      try {
        ...
        logger.debug("[AliExpress] Fetch attempt %d with UA: %s...", i + 1, headers["User-Agent"].slice(0, 60));

        const res = await fetchWithRetry(url, headers, this.cookieJar);
        lastStatus = res.status;

        if (!res.ok) {
          logger.warn("[AliExpress] HTTP %d", res.status);
          if (res.status === 403 || res.status === 503) {
            const delay = randomInt(2_000, 5_000);
            logger.debug("[AliExpress] %d, waiting %dms...", res.status, delay);
            await sleep(delay);
            continue;
          }
          continue;
        ...
        if (antiBotError) {
          logger.warn("[AliExpress] Anti-bot detected: %s", antiBotError);
          const delay = randomInt(2_000, 5_000);
          await sleep(delay);
          continue;
        }
        ...
        logger.warn("[AliExpress] Fetch attempt %d failed: %s", i + 1, error.message);
  ...
  private extractImages(result: unknown, pageHtml: string): string[] {
    ...
    logger.debug("[AliExpress] API images: %d", apiGallery.length);

    const htmlImages = collectAllImages(pageHtml);
    for (const img of htmlImages) {
      if (img.startsWith("http")) all.add(img);
    }
    logger.debug("[AliExpress] HTML images: %d", htmlImages.length);
    ...
    if (embeddedData) {
      ...
      logger.debug("[AliExpress] Embedded images: %d", embeddedImages.length);
    }

    const resultArray = [...all];
    logger.debug("[AliExpress] Total unique images: %d", resultArray.length);
    return resultArray;
  }
  ...
  private extractVariants(result: unknown, fallbackPrice: number): ProductVariant[] {
    ...
    if (skuPaths.length === 0) {
      ...
      if (variants.length > 0) {
        logger.debug("[AliExpress] Extracted %d variants from attributes", variants.length);
        return variants;
      }
    ...
    logger.debug("[AliExpress] Extracted %d variants from SKU paths", variants.length);
    return variants;
  }
    ...
    for (const { name, re } of patterns) {
      ...
      if (match?.[1]) {
        try {
          ...
          if (data) {
            logger.debug("[AliExpress] Extracted embedded data from %s", name);
            return data;
          }
        } catch (e) {
          logger.debug({ err: e }, "[AliExpress] Failed to parse %s", name);
        }
      }
    }
    return null;
  ...
  public async extract(url: string, rawHtml?: string, customPrompt?: string): Promise<NormalizedProduct> {
    ...
    if (matched) {
      if (matched.success && matched.product) {
        logger.info("[AliExpress] Test dataset hit for: %s", matched.product.title);
    ...
    logger.info("[AliExpress] Product ID: %s", productId);
    ...
    try {
      ...
      logger.info('[AliExpress] Page title: "%s"', pageTitle);
    } catch (error: any) {
      logger.warn("[AliExpress] Page fetch warning: %s", error.message);
    }
    ...
    try {
      ...
      logger.info("[AliExpress] API fetch successful");
    } catch (error: any) {
      logger.warn("[AliExpress] API fetch failed: %s", error.message);
    }
    ...
    logger.info('[AliExpress] Title: "%s"', title);
    ...
    logger.info("[AliExpress] Successfully parsed product: %s | Images: %d | Price: %d %s", product.title, gallery.length, product.price, product.currency);
    return product;
  }
}