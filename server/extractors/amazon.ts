...
function extractAmazonEmbeddedJson(html: string): any | null {
  ...
  if (twisterMatch?.[1]) {
    try {
      ...
      if (data) {
        logger.debug(`[AmazonExtractor] Found twister-js-init-dpx-data JSON`);
        return data;
      }
    } catch {
      // Continue
  ...
  if (colorImagesMatch?.[1]) {
    try {
      ...
      if (data) {
        logger.debug(`[AmazonExtractor] Found colorImages JSON`);
  ...
  if (dimMatch?.[1]) {
    try {
      ...
      if (data) {
        logger.debug(`[AmazonExtractor] Found dimensionValuesDisplayData JSON`);
  ...
  if (initialStateMatch?.[1]) {
    try {
      ...
      if (data) {
        logger.debug(`[AmazonExtractor] Found window.__INITIAL_STATE__ JSON`);
        return data;
      }
    } catch {
      // Continue
  ...
  if (pMatch?.[1]) {
    try {
      ...
      if (data) {
        logger.debug(`[AmazonExtractor] Found window.P JSON`);
        return data;
      }
    } catch {
      // Continue
  ...
  if (imgBlockMatch?.[1]) {
    try {
      ...
      if (data) {
        logger.debug(`[AmazonExtractor] Found imageBlockVariations JSON`);
...
function extractAmazonImagesEnhanced(html: string, productJsonLd: any | null, embeddedJson: any | null): string[] {
  ...
  logger.debug(`[AmazonExtractor] Strategy 1 (original extractAmazonImages) found ${originalImages.length} images`);
  ...
  if (embeddedJson?.colorImages?.initial) {
    ...
    logger.debug(`[AmazonExtractor] Strategy 2 (colorImages.initial) found ${colorImgUrls.length} images`);
  }
  ...
  if (embeddedJson?.imageBlockVariations) {
    ...
    logger.debug(`[AmazonExtractor] Strategy 3 (imageBlockVariations) found ${imgBlockUrls.length} images`);
  }
  ...
  if (ogImage && ogImage.startsWith("http")) {
    allUrls.add(ogImage);
    logger.debug(`[AmazonExtractor] Strategy 4 (og:image) found 1 image`);
  }
  ...
  if (twImage && twImage.startsWith("http")) {
    allUrls.add(twImage);
    logger.debug(`[AmazonExtractor] Strategy 5 (twitter:image) found 1 image`);
  }
  ...
  logger.debug(`[AmazonExtractor] Strategy 6 (a-dynamic-image) found ${dynamicCount} images`);
  ...
  logger.debug(`[AmazonExtractor] Strategy 7 (m.media-amazon.com regex) found ${mediaCount} images`);
  ...
  logger.debug(`[AmazonExtractor] Strategy 8 (ssl-images-amazon.com) found ${sslCount} images`);

  const result = [...allUrls];
  logger.debug(`[AmazonExtractor] Total unique images extracted: ${result.length}`);
  return result;
}
...
function extractAmazonPriceEnhanced(html: string, productJsonLd: any | null, embeddedJson: any | null): { amount: number; raw: string; currency: string } | null {
  ...
  if (originalPrice) {
    logger.debug(`[AmazonExtractor] Price found via original extractAmazonPrice: ${originalPrice.amount}`);
  ...
  if (jsonLdPrice) {
    logger.debug(`[AmazonExtractor] Price found via JSON-LD: ${jsonLdPrice.amount}`);
    return jsonLdPrice;
  }
  ...
  if (priceToPayMatch?.[1]) {
    ...
    if (!isNaN(amount) && amount > 0) {
      logger.debug(`[AmazonExtractor] Price found via priceToPay: ${amount}`);
      return { amount, raw, currency: detectCurrency(raw) };
    }
  }
  ...
  if (corePriceMatch?.[1]) {
    ...
    const amount = normalizePrice(raw);
    if (!isNaN(amount) && amount > 0) {
      logger.debug(`[AmazonExtractor] Price found via corePriceDisplay: ${amount}`);
      return { amount, raw, currency: detectCurrency(raw) };
    }
  }
  ...
  if (apexMatch) {
    ...
    const amount = normalizePrice(raw);
    if (!isNaN(amount) && amount > 0) {
      logger.debug(`[AmazonExtractor] Price found via apex_desktop: ${amount}`);
      return { amount, raw, currency: detectCurrency(raw) };
    }
  }
  ...
  if (wholeMatch) {
    ...
    if (!isNaN(amount) && amount > 0) {
      logger.debug(`[AmazonExtractor] Price found via a-price-whole/fraction: ${amount}`);
      return { amount, raw, currency: detectCurrency(raw) };
    }
  }
  ...
  if (hiddenPriceMatch?.[1]) {
    ...
    const amount = parseFloat(raw);
    if (!isNaN(amount) && amount > 0) {
      logger.debug(`[AmazonExtractor] Price found via hidden JSON: ${amount}`);
      return { amount, raw, currency: "USD" };
    }
  }

  logger.info(`[AmazonExtractor] All price strategies failed`);
  return null;
}
  ...
  public async extract(url: string, rawHtml?: string, customPrompt?: string): Promise<NormalizedProduct> {
    ...
    if (matched) {
      if (matched.success && matched.product) {
        logger.info(`[AmazonExtractor] Deep-crawl offline database hit for: ${matched.product.title}`);
    ...
    if (!this.homepageBootstrapped) {
      try {
        logger.info(`[AmazonExtractor] Bootstrapping Amazon homepage cookies...`);
        ...
        if (homeRes.ok) {
          logger.info(`[AmazonExtractor] Homepage bootstrap successful, cookies stored: ${this.cookieJar.getJar().size}`);
          this.homepageBootstrapped = true;
          await sleep(randomInt(1_000, 3_000));
        }
      } catch (err: any) {
        logger.warn(`[AmazonExtractor] Homepage bootstrap failed (non-fatal): ${err.message}`);
      }
    }
    ...
    if (!html) {
      try {
        logger.info(`[AmazonExtractor] Fetching product page: ${cleanUrl}`);
        html = await this.fetchAmazonPage(cleanUrl);
      } catch (err: any) {
        logger.warn(`[AmazonExtractor] Primary fetch failed: ${err.message}`);
        ...
        if (asin) {
          ...
          for (const altUrl of altUrls) {
            ...
            logger.info(`[AmazonExtractor] Retrying with alternate URL: ${altUrl}`);
            try {
              html = await this.fetchAmazonPage(altUrl);
              if (html) break;
            } catch (altErr: any) {
              logger.warn(`[AmazonExtractor] Alternate URL failed: ${altErr.message}`);
            }
          }
        }
      }
  ...
  private async fetchAmazonPage(url: string): Promise<string> {
    ...
    for (let i = 0; i < 3; i++) {
      try {
        ...
        logger.debug(`[AmazonExtractor] Fetch attempt ${i + 1} with UA: ${headers["User-Agent"].slice(0, 60)}...`);

        const res = await fetchWithRetry(url, headers, this.cookieJar);
        lastStatus = res.status;

        if (!res.ok) {
          logger.warn(`[AmazonExtractor] HTTP ${res.status} from Amazon`);
          if (res.status === 403 || res.status === 503) {
            const delay = randomInt(2_000, 5_000);
            logger.debug(`[AmazonExtractor] ${res.status} detected, waiting ${delay}ms before retry...`);
            await sleep(delay);
            continue;
          }
          continue;
        ...
        const antiBotError = detectAmazonAntiBot(html);
        if (antiBotError) {
          logger.warn(`[AmazonExtractor] Anti-bot detected: ${antiBotError}`);
          const delay = randomInt(2_000, 5_000);
          await sleep(delay);
          continue;
        }
        ...
        logger.warn(`[AmazonExtractor] Fetch attempt ${i + 1} failed: ${error.message}`);
