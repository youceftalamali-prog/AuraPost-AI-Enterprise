  ...
  if (mskuMatch?.[1]) {
    try {
      ...
      if (mskuData) {
        ...
        logger.info(`[EBayExtractor] Extracted ${variants.length} variants from MSKU data`);
      }
    } catch (err: any) {
      logger.warn(`[EBayExtractor] MSKU parse failed: ${err.message}`);
    }
  }
  ...
  public async extract(url: string, rawHtml?: string, customPrompt?: string): Promise<NormalizedProduct> {
    ...
    if (matched) {
      if (matched.success && matched.product) {
        logger.info(`[EBayExtractor] Deep-crawl offline database hit for: ${matched.product.title}`);
    ...
    if (!this.homepageBootstrapped) {
      try {
        logger.info(`[EBayExtractor] Bootstrapping eBay homepage cookies...`);
        ...
        if (homeRes.ok) {
          logger.info(`[EBayExtractor] Homepage bootstrap successful, cookies stored: ${this.cookieJar.getJar().size}`);
          this.homepageBootstrapped = true;
          // Random delay after homepage to appear human
          await sleep(randomInt(1_000, 3_000));
        }
      } catch (err: any) {
        logger.warn(`[EBayExtractor] Homepage bootstrap failed (non-fatal): ${err.message}`);
      }
    }
    ...
    if (!html) {
      try {
        logger.info(`[EBayExtractor] Fetching product page: ${cleanUrl}`);
        html = await this.fetchEbayPage(cleanUrl);
      } catch (err: any) {
        logger.warn(`[EBayExtractor] Primary fetch failed: ${err.message}`);
        ...
        if (itemId && err.message?.includes("403")) {
          ...
          logger.info(`[EBayExtractor] Retrying with alternate URL format: ${altUrl}`);
          try {
            html = await this.fetchEbayPage(altUrl);
          } catch (altErr: any) {
            logger.warn(`[EBayExtractor] Alternate URL also failed: ${altErr.message}`);
          }
        }
      }
    }
    ...
    if (embeddedJson) {
      logger.info(`[EBayExtractor] Found embedded eBay JSON data`);
    }
  ...
  private async fetchEbayPage(url: string): Promise<string> {
    ...
    for (let i = 0; i < 3; i++) {
      try {
        ...
        logger.info(`[EBayExtractor] Fetch attempt ${i + 1} with UA: ${headers["User-Agent"].slice(0, 60)}...`);

        const res = await fetchWithRetry(url, headers, this.cookieJar);
        lastStatus = res.status;

        if (!res.ok) {
          logger.warn(`[EBayExtractor] HTTP ${res.status} from eBay`);
          if (res.status === 403) {
            // Add randomized delay before retry
            const delay = randomInt(2_000, 5_000);
            logger.info(`[EBayExtractor] 403 detected, waiting ${delay}ms before retry...`);
            await sleep(delay);
            continue;
          }
          continue;
        ...
        const antiBotError = detectEbayAntiBot(html);
        if (antiBotError) {
          logger.warn(`[EBayExtractor] Anti-bot detected: ${antiBotError}`);
          const delay = randomInt(2_000, 5_000);
          await sleep(delay);
          continue;
        }
        ...
        logger.warn(`[EBayExtractor] Fetch attempt ${i + 1} failed: ${error.message}`);
