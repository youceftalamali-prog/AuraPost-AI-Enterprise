...
export async function scrapeShopifyProduct(
  ...
  logger.info(`[Shopify Extractor] Loading live JSON endpoint with 4s timeout: ${jsonUrl}`);
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (rawPayload && (rawPayload.product || rawPayload.id)) {
        logger.info(`[Shopify Extractor] Successfully fetched direct shopify config for item: ${rawPayload.product?.title || "unknown"}`);
        return normalizeShopifyProduct(rawPayload, url);
      }
    }

    logger.warn({ status: response.status }, "[Shopify Extractor] Direct JSON endpoint returned non-ok status. Invoking AI fallback...");
  } catch (err) {
    logger.warn({ err }, "[Shopify Extractor] Network request failed or timed out. Attempting AI crawling fallback...");
  }
...
async function scrapeWithAIFallback(url: string, aiClient: GoogleGenAI | null): Promise<ExtractedProduct> {
  ...
  if (!aiClient) {
    return {
      ...
      variants: [],
      availability: false,
      extractionError: "No AI provider configured. Please configure an AI provider to extract product data.",
    };
  }

  try {
    logger.info("[Shopify Extractor] Running Gemini AI structured scraping on HTML page...");
    ...
    try {
      ...
      logger.debug({ err: e }, "[Shopify Extractor] HTML page fetch failed or timed out:");
    }
    ...
    if (txt) {
      ...
      const cleanImages = (data.images || []).filter((img: string) => img.startsWith("http"));
    ...
    logger.error({ err: error }, "[Shopify Extractor] Gemini crawling fallback error:");
  }
