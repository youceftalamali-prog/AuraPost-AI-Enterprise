...
import { setAgentWorkflowHandoff } from './handoff';
...
const copy = {
  ar: {
    eyebrow: 'وكيل Aura الذكي', title: 'حوّل منتجك إلى حملة تسويقية كاملة',
    ...
    placeholder: 'ألصق رابط المنتج وأضف تعليماتك، أو صف الحملة التي تريدها…', run: 'ابدأ مع Aura',
    sourceTitle: 'كيف تريد أن تبدأ؟', url: 'رابط المنتج', image: 'صورة المنتج', saved: 'منتج محفوظ',
    descriptionMode: 'وصف مكتوب', templates: 'قوالب جاهزة حسب مجال المنتج',
    templatesHint: 'اختر قالباً، ثم أرسل المصدر إلى الوكيل ليكمل العمل.', selected: 'القالب المختار',
  },
  fr: {
    eyebrow: 'Agent intelligent Aura', title: 'Transformez votre produit en campagne complète',
    ...
    placeholder: 'Collez le lien du produit et ajoutez vos instructions…', run: 'Démarrer avec Aura',
    sourceTitle: 'Comment souhaitez-vous commencer ?', url: 'Lien produit', image: 'Image produit', saved: 'Produit enregistré',
    descriptionMode: 'Description', templates: 'Modèles par catégorie',
    templatesHint: 'Choisissez un modèle, puis fournissez la source à l’agent.', selected: 'Modèle sélectionné',
  },
  en: {
    eyebrow: 'Aura intelligent agent', title: 'Turn one product into a complete campaign',
    ...
    placeholder: 'Paste the product link and add your campaign instructions…', run: 'Start with Aura',
    sourceTitle: 'How would you like to start?', url: 'Product URL', image: 'Product image', saved: 'Saved product',
    descriptionMode: 'Description', templates: 'Templates by product category',
    templatesHint: 'Choose a template, then give the product source to the agent.', selected: 'Selected template',
  },
...
const categoryIcons: Record<string, ComponentType<{ className?: string }>> = {
  Jewelry: Gem, Fashion: Shirt, Beauty: Sparkles, Electronics: Boxes,
  Home: WandSparkles, Fitness: Video, Food: ImageIcon, Universal: ArrowUpRight,
};

export function AgentHome({ locale, selectedTemplate, onSelectTemplate, onStart }: AgentHomeProps) {
  const [prompt, setPrompt] = useState('');
  const text = copy[locale];
  const isRtl = locale === 'ar';

  const startWorkflow = (mode: AgentSourceMode, value: string) => {
    setAgentWorkflowHandoff({
      mode,
      prompt: value,
  ...
  const submitPrompt = (event: FormEvent) => {
    ...
    if (!trimmed) return;
    startWorkflow(selectedTemplate ? 'template' : 'description', trimmed);
  };
    ...
    <div dir={isRtl ? 'rtl' : 'ltr'} className="space-y-10">
      <section className="relative overflow-hidden rounded-[28px] border border-indigo-400/20 bg-[#11131d] px-5 py-8 shadow-2xl sm:px-8 lg:px-12 lg:py-12">
        ...
        <div className="relative mx-auto max-w-4xl text-center">
          ...
          <h1 className="mt-5 text-3xl font-bold tracking-tight text-white sm:text-5xl">{text.title}</h1>
          <p className="mx-auto mt-4 max-w-2xl text-sm leading-7 text-slate-300 sm:text-base">{text.description}</p>
          ...
          {selectedTemplate && (
            <div className="mx-auto mt-5 inline-flex items-center gap-2 rounded-xl border border-emerald-400/20 bg-emerald-400/10 px-3 py-2 text-xs text-emerald-200">
              <span>{text.selected}:</span><strong>{selectedTemplate.category} · {selectedTemplate.title}</strong>
            </div>
          )}
          ...
          <form onSubmit={submitPrompt} className="mx-auto mt-7 flex max-w-3xl flex-col gap-3 rounded-2xl border border-white/10 bg-black/20 p-3 shadow-xl sm:flex-row">
            <textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder={text.placeholder} rows={2}
            ...
            <button type="submit" className="inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-indigo-500 to-emerald-500 px-5 py-3 text-sm font-semibold text-white transition hover:brightness-110">
              <WandSparkles className="h-4 w-4" />{text.run}
            </button>
          </form>
          ...
          <div className="mt-7">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              {sourceOptions.map(({ mode, icon: Icon, labelKey }) => (
                <button key={mode} type="button" onClick={() => startWorkflow(mode, prompt.trim())}
                  ...
                  <Icon className="h-4 w-4 text-indigo-300 transition group-hover:text-emerald-300" />{text[labelKey]}
                </button>
              ))}
            </div>
          </div>
      ...
      <section>
        <div className="mb-5 flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
          <div><h2 className="text-xl font-semibold text-white">{text.templates}</h2><p className="mt-1 text-sm text-slate-400">{text.templatesHint}</p></div>
        </div>
          ...
          {templates.map((template) => {
            ...
            return (
              <button key={template.id} type="button" onClick={() => onSelectTemplate(template)}
                ...
                <div className={`h-24 bg-gradient-to-br ${template.accent} p-4`}><Icon className="h-6 w-6 text-white" /></div>
